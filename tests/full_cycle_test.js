...
const TEST_FILE_NAME = 'test_file.js';
const TEST_FILE_DIR = path.join(__dirname, 'test_data');
const TEST_FILE_PATH = path.join(TEST_FILE_DIR, TEST_FILE_NAME);


async function runFullCycleTest() {
  console.log('--- Запуск комплексного E2E теста ---');
  ...
  // --- Шаг 4: RAG-запрос ---
  const question = "/**\\n * Расскажи в деталях, что делает функция add(a, b)";
