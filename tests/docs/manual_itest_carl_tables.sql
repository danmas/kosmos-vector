...
-- Пример 2: Таблица продуктов
CREATE TABLE aaaaa.products (
...
-- Пример 3: Функция для получения активных пользователей
CREATE OR REPLACE FUNCTION aaaaa.get_active_users()
RETURNS TABLE (
    user_id INTEGER,
    username VARCHAR,
    email VARCHAR,
...
-- Пример 4: Представление для доступных продуктов
CREATE OR REPLACE VIEW aaaaa.available_products AS
SELECT 
    p.product_id,
    p.name,
    p.description,
    ...
    p.stock_quantity,
    p.category_id
FROM aaaaa.products p
WHERE p.is_available = TRUE AND p.stock_quantity > 0;

-- Пример 5: Процедура для обновления стока продукта
CREATE OR REPLACE PROCEDURE aaaaa.update_product_stock(
...
AS $$
BEGIN
    UPDATE aaaaa.products
    SET stock_quantity = p_quantity,
        is_available = CASE WHEN p_quantity > 0 THEN TRUE ELSE FALSE END
...
-- Пример 6: Триггер для логирования изменений пользователей
CREATE OR REPLACE FUNCTION aaaaa.log_user_changes()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'UPDATE' THEN
        INSERT INTO aaaaa.user_changes_log(user_id, change_type, changed_at)
...
AFTER UPDATE OR DELETE ON aaaaa.users
FOR EACH ROW EXECUTE FUNCTION aaaaa.log_user_changes();

-- Пример 7: Индекс для улучшения производительности поиска по имени продукта
CREATE INDEX idx_products_name ON aaaaa.products(name);
