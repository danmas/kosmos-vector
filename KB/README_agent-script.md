...
## Как это работает (Конвейер запроса)

Процесс обработки вопроса проходит через несколько стадий:

1. **Поиск похожих вопросов (векторный поиск):**
   - Система векторизует вопрос пользователя.
   - Выполняет поиск в таблице `agent_script` по эмбеддингам.
   - Если найден похожий валидный вопрос (`is_valid = true`, similarity >= 0.95), предлагается использовать существующий скрипт.
...
3. **Выполнение в Sandbox:**
   ...
   - **Таймаут:** Ограничение времени выполнения (по умолчанию 5 секунд).
...
Диалоговое окно предоставляет три вкладки для просмотра результатов:

| Вкладка | Описание | Пример содержимого |
|---------|----------|-------------------|
| **Interpretation** | Текстовый ответ на вопрос на человеческом языке | «В проекте найдено 12 SQL-функций, работающих с таблицей пользователей» |
| **Agent Script** | Просмотр сгенерированного JS-кода с подсветкой синтаксиса | `async function execute(contextCode) { ... }` |
| **Raw Data** | Результат выполнения SQL-запроса в формате JSON | `[{"source": "api_auct", "target": "helper_fn", "type": "calls"}]` |

Во вкладке **Agent Script** можно отредактировать код и запустить его вручную (кнопка *Run*), а также скопировать код в буфер обмена.
...
3. **Интеграция с Knowledge Graph:**
   - Кнопка **«Apply to Filter»** извлекает идентификаторы (имена функций, пути файлов) из результатов запроса.
...
- [README_links.md](./README_links.md) — Граф зависимостей и типы связей
- [README_EMBEDDING.md](./README_EMBEDDING.md) — Система эмбеддингов и векторизации
- [README_REST.md](./README_REST.md) — Полный REST API reference
