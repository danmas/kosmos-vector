 ...
 */
async function loadTsFunctionsFromFile(filePath, contextCode, dbService, pipelineState = null, mode = 'incremental') {
    const filename = path.basename(filePath);
    console.log(`[TS-Loader] Обработка файла: ${filename} (mode=${mode})`);
    ...
    if (mode === 'incremental') {
        try {
            ...
            if (!changeResult.changed) {
                console.log(`[TS-Loader] Файл ${filename} не изменился (${changeResult.status}), пропускаем`);
                report.skipped = true;
                report.skipReason = changeResult.status;
                report.fileId = changeResult.fileId;
                return report;
            }
            tsContent = changeResult.content;
            fileHash = changeResult.newHash;
            report.fileId = changeResult.fileId;
        } catch (err) {
            console.warn(`[TS-Loader] Ошибка инкрементальной проверки: ${err.message}`);
            tsContent = null;
        }
    }
    ...
    // Регистрация файла
    try {
        const { id: fileId, isNew } = await dbService.saveFileInfo(filename, tsContent, filePath, contextCode, fileHash);
    ...
    for (const code of Object.values(linkTypeMap)) {
        try {
            ...
            if (res.rows.length > 0) {
                ...
                console.warn(`[TS-Loader] Тип связи '${code}' не найден в link_type`);
            }
        } catch (err) {
            console.error(`[TS-Loader] Ошибка при получении link_type '${code}':`, err.message);
        }
    }
    ...
    if (mode === 'incremental' && report.fileId) {
        try {
                            ...
                            for (const [key, code] of Object.entries(linkTypeMap)) {
                                ...
                                for (const target of targets) {
                                    try {
                                        ...
                                        console.error(`[TS-Loader] Ошибка link ${entity.full_name} -> ${target}:`, err.message);
                                    }
                                }
                            }
                        } catch (err) {
                            console.error(`[TS-Loader] Ошибка парсинга L1 для ${entity.full_name}: ${err.message}`);
                        }
                    }
                }
            });
            ...
            console.log(`[TS-Loader] Инкрементальный итог: created=${entityReport.created}, updated=${entityReport.updated}, unchanged=${entityReport.unchanged}, deleted=${entityReport.deleted}`);
            return report;
        } catch (err) {
            console.error(`[TS-Loader] Ошибка инкрементальной обработки: ${err.message}`);
        }
    }
                ...
                if (['function', 'method', 'arrow', 'class'].includes(entity.type)) {
                    try {
                        ...
                        if (l1Result && entityReport.aiItemId) {
                            ...
                            for (const [key, code] of Object.entries(linkTypeMap)) {
                                ...
                                for (const target of targets) {
                                    try {
                                        ...
                                        console.error(`[TS-Loader] Ошибка link ${entity.full_name} -> ${target} (${code}):`, err.message);
                            ...
                            if (linksCount > 0) {
                                console.log(`[TS-Loader] Сохранено ${linksCount} связей для ${entity.full_name}`);
                            }
                        }
